...
class ProxyManager:
    ...
    def _format_proxy_for_telethon(self, proxy_str: str):
        """Format proxy string for Telethon (returns tuple)"""
        if not proxy_str:
            return None
        
        # If it already has a scheme, parse it
        if proxy_str.startswith('http://'):
            proxy_str = proxy_str[7:]  # Remove 'http://'
        
        if proxy_str.startswith('https://'):
            proxy_str = proxy_str[8:]  # Remove 'https://'
        ...
        if '@' in proxy_str:
            ...
            return ('http', host, int(port), user, password)
        else:
            # Format: host:port
...
class AccountManager:
    ...
    async def create_desktop_session(self, phone: str, update: Update, user_id: int) -> bool:
        ...
        try:
            ...
            # Set proxy - FIX: Use Telethon proxy format
            if account.proxy:
                try:
                    ...
                    await update.message.reply_text("❌ Proxy configuration error.")
                    return False
            ...
            try:
                ...
                # Request OTP with better error handling
                try:
                    ...
                    if await client.is_user_authorized():
                        ...
                        await update.message.reply_text(
                            f"✅ *Already Logged In!*\n\n"
                            f"Account `{phone}` is already authorized.\n"
                    ...
                    await update.message.reply_text(
                        ...
                        f"_Note: If you don't receive code, try again in 2 minutes_",
                        parse_mode='Markdown'
                    )
                    ...
                    wait_time = e.seconds
                    await update.message.reply_text(f"⏳ Wait {wait_time}s ({wait_time//60} minutes) before retrying.")
                    return False
                except PhoneNumberUnoccupiedError:
                    await update.message.reply_text("❌ Phone number not registered on Telegram.")
                    return False
                except PhoneNumberBannedError:
                    await update.message.reply_text("❌ Phone number is banned by Telegram.")
                    account.status = AccountStatus.BANNED
                    self._save_accounts()
                    return False
                except Exception as e:
                    console.print(f"[red]❌ Error requesting OTP: {e}[/red]")
                    await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
                    return False
                    
            except Exception as e:
                console.print(f"[red]❌ Connection error: {e}[/red]")
                await update.message.reply_text(f"❌ Connection error: {str(e)[:100]}")
                return False
    ...
    async def verify_otp(self, phone: str, otp: str, update: Update, user_id: int) -> bool:
        """
        Verify OTP code with 2FA handling
        ...
        try:
            ...
            try:
                ...
                await self._handle_successful_login(phone, client, update, user_id)
                return True
                
            except SessionPasswordNeededError:
                # 2FA required - store for password input
                ...
                await update.message.reply_text("❌ Code expired. Start over with /add.")
    ...
    async def _handle_successful_login(self, phone: str, client: TelegramClient, update: Update, user_id: int):
        ...
        try:
            ...
            account.country = self._get_country_from_phone(phone)
            account.is_premium = getattr(me, 'premium', False)
            
            # Check if 2FA is enabled
            try:
                # Try to get 2FA settings
                account.two_factor_enabled = await self._check_2fa_enabled(client)
            except:
    ...
    async def _check_2fa_enabled(self, client: TelegramClient) -> bool:
    ...
    async def verify_password(self, phone: str, password: str, update: Update, user_id: int) -> bool:
        ...
        try:
            ...
            await update.message.reply_text("🔐 Verifying 2FA password...", parse_mode='Markdown')
            ...
            try:
                ...
                await update.message.reply_text(
                    ...
                    f"Account `{phone}` is now active with 2FA enabled.",
                    parse_mode='Markdown'
                )
                ...
                await update.message.reply_text(f"❌ Invalid 2FA password. Try again.")
                return False
    ...
    def get_stats(self) -> Dict:
        ...
        with_2fa = sum(1 for a in self.accounts.values() if a.two_factor_enabled)
    ...
    async def _execute_report(self, account: TelegramAccount, job: ReportJob) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "2fa": account.two_factor_enabled,
...
• With 2FA: {account_stats['accounts_with_2fa']}
...
"""
    ...
    async def accounts_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for i, acc in enumerate(accounts[:10], 1):
            ...
            text += f"   • 2FA: {'🔒' if acc.two_factor_enabled else '❌'}\n\n"
...
• Full 2FA (Two-Step Verification) support
"""
